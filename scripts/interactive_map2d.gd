...
func _ready() -> void:
	...
	for polygon in child_count:
		apply_overlay(area2d.get_child(polygon).polygon,area2d.get_child(polygon).global_position)
...
func apply_overlay(vec2_array:PackedVector2Array, pos:Vector2) -> void:
	var instanced_polygon_overlay = polygon_overlay.instantiate()
	#var instanced_script = preload("res://scripts/interactive_map2d_polygon_overlay.gd").new()
