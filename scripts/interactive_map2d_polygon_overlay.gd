...
const uv_speed_out:float = 0.8
const uv_speed_in:float = 0.5
...
func _process(delta: float) -> void:
	if pm == true:
		if collision_index == polygon_index:
			uv_addition = min(uv_addition + uv_speed_in * delta, 2.0)
			material.set_shader_parameter("uv_addition", uv_addition)
		else:
			uv_addition = max(uv_addition - (uv_speed_out+switch_speed) * delta, 0.0)
			material.set_shader_parameter("uv_addition", uv_addition)
	else:
		if collision_index != polygon_index:
			uv_addition = max(uv_addition - uv_speed_out * delta, 0.0)
			material.set_shader_parameter("uv_addition", uv_addition)
